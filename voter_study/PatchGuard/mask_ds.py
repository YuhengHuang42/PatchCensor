...
parser.add_argument("--result", type=str, default="result_ds_imagenet_val.pt", help="where to save the result")
...
def get_dataset(ds,data_dir):
    if ds in ['imagenette','imagenet', "PartImageNet"]:
        #ds_dir=os.path.join(data_dir,'validation')
        ds_dir = data_dir
        normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                         std=[0.229, 0.224, 0.225])
        dataset_ = torchvision.datasets.ImageFolder(root=ds_dir, transform=transforms.Compose([
...
if args.ds:#ds
    ...
    total = 0
    correct_list = list()
    cert_correct_list = list()
    cert_incorrect_list = list()
    file_list = list()
    with torch.no_grad():
        for all_inputs in tqdm(val_loader):
            inputs = all_inputs[0]
            targets = all_inputs[1]
            file_name = all_inputs[2]
            inputs, targets = inputs.to(device), targets.to(device)
            total += targets.size(0)
            predictions,  certyn = ds(inputs, net,args.band_size, args.patch_size, num_cls,threshold = 0.2)
            correct += (predictions.eq(targets)).sum().item()
            correct_list += (predictions.eq(targets)).reshape(-1).tolist()
            cert_correct += (predictions.eq(targets) & certyn).sum().item()
            cert_correct_list += (predictions.eq(targets) & certyn).reshape(-1).tolist()
            cert_incorrect += (~predictions.eq(targets) & certyn).sum().item()
            cert_incorrect_list += (~predictions.eq(targets) & certyn).reshape(-1).tolist()
    ...
    save_result = {
        ...
        "cert_correct_list": cert_correct_list,
        "cert_incorrect_list": cert_incorrect_list,
        "file_list": file_list
    }
